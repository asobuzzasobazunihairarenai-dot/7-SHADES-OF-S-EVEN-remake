...
// 【2026-09-08 追記・続き480】上の原則（アプリ側の最長の立て直しより必ず後に諦める）は
// 正しかったが、**当時数え落としていた待ちが2つあった**——委任の応答待ち
// （main.js の giveUpTimer）と接触で奪う札の選択待ち（CONTACT_PICK_WAIT_MAX_MS）が
// **どちらも90秒**。100秒では、その90秒の後にアプリが実際に動き出すまでの数秒を待てず、
// **立て直せる場面まで停止と判定していた**（実測: turn 3 で arrivalProcessing:true のまま
// 100秒でFAIL。アプリ側は90秒で諦めて先へ進む作りになっている）。
// 90秒＋十分な余裕として150秒にする。**アプリ側に90秒より長い待ちを足したら、ここも必ず伸ばすこと。**
// なお「手は動いているのにターンが進まない」堂々巡りは下の TURN_STALL_MAX_MS が別に見張るので、
// ここを伸ばしても本物の異常を見逃すことにはならない。
