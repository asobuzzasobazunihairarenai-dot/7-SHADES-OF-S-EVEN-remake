...
const SLIDERS = [
  ["--vic-speed", "全体の速さ（大きいほど速い）", 0.5, 2.5, 0.05, 0.85],
  ["--vic-color-step", "1色ごとの間隔", 0.3, 2.5, 0.05, 1],
  ["--vic-gather-speed", "キューブへ集まる速さ", 0.4, 2.5, 0.05, 1],
  ["--vic-stream", "光の帯・霧の量", 0.2, 2.5, 0.05, 1.8],
  ["--vic-pulse-count", "脈動の回数", 1, 6, 1, 3],
  ["--vic-pulse-power", "脈動の強さ", 0.3, 2.5, 0.05, 1.6],
  ["--vic-cube-white", "脈動の白い芯（0で白い丸が消える）", 0, 1.5, 0.05, 0],
  ["--vic-shake", "画面振動の強さ", 0, 3, 0.05, 1.6],
...
// プリセット。値は CSS変数そのもの。
// 【既定＝以前の「派手版」】ユーザーが派手版を選んだので、style.css の :root 自体を
// その値にした（2026-08-30）。なので「標準版」＝いまの既定＝派手版。以前の標準の値は
// 「控えめ版」として残してある。
const PRESETS = {
  標準版: {},
